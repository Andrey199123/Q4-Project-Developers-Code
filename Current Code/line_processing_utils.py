...
def determine_roi_presence(mask, frame_width, frame_height):
    ...
    print(np.count_nonzero(left_roi_section))
...
def determine_direction(left_detected, right_detected):
    ...
    if left_detected and right_detected:
        ...
        return "Unknown"
def detect_blue_in_rois(frame, frame_width, frame_height):
    """Detects blue tape in left and right ROIs using HSV color space"""
    hsv = cv2.cvtColor(frame, cv2.COLOR_BGR2HSV)
    lower_blue = np.array([90, 100, 50])
    upper_blue = np.array([150, 255, 255])
    blue_mask = cv2.inRange(hsv, lower_blue, upper_blue)
    
    # Apply ROI mask to focus on relevant regions
    roi_blue_mask, _ = ROI(blue_mask, frame_width, frame_height)
    
    # Check for significant blue presence
    left_detected, right_detected = determine_roi_presence(roi_blue_mask, frame_width, frame_height)
    return left_detected, right_detected

def get_blue_direction(left_detected, right_detected):
    """Determine turn direction based on blue detection"""
    if left_detected and right_detected:
        return "Middle"
    if left_detected:
        return "Left"
