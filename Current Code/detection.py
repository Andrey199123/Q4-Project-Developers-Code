...
def process_frame(frame):
    ...
    # Obstacle check (must come after Martian detection)
    if ultrasonic:
    ...
    if path_detected:
        # Blue detection takes priority
        blue_left, blue_right = line_processing_utils.detect_blue_in_rois(frame, frame_width, frame_height)
        blue_direction = line_processing_utils.get_blue_direction(blue_left, blue_right)
        
        if blue_direction:
            # Use blue-based direction
            line_detection_state["direction"] = blue_direction
            print(f"Blue detected - Direction: {blue_direction}")
        else:
            # Fall back to saturation-based detection
            processed_mask, updated_state = line_processing_utils.get_direction_from_roi_saturation(
                frame, line_detection_state
            )
            line_detection_state.update(updated_state)
            print(f"Detected Direction (ROI Saturation): {line_detection_state['direction']}")

        # Visualization (keep original ROI overlay)
        final_display_frame = cv2.cvtColor(processed_mask, cv2.COLOR_GRAY2BGR) if not blue_direction else frame.copy()
        ...
        colored_roi_overlay[roi_mask_for_viz == 255] = [0, 255, 0]  # Green ROI overlay
        ...
        if not action_flags["CURRENTLY_RUNNING_PROCEDURE"]:
            ...
            threading.Thread(
                ...
                args=(line_detection_state["direction"], action_flags, turn_parameters)
            ).start()

        return cv2.addWeighted(apply_lane_detection(frame), 0.7, final_display_frame, 0.3, 0)

    # Fallback for non-path frames
