...
# --- Helper Functions (keep saturation_mask, ROI, determine_roi_presence, determine_direction) ---

def saturation_mask(frame, saturation_threshold=0.2, min_percent=10, min_region_size=600):
    """
    Applies a saturation threshold to the frame and filters out small noisy regions.
    Returns a single-channel binary mask where white (255) indicates high saturation.
    """
...
def determine_direction(left_detected, right_detected):
    ...
    if left_detected and right_detected:
        ...
        return "Right"  # White detected on left, need to turn right to center
    elif right_detected:
        return "Left"   # White detected on right, need to turn left to center
    else:
        return "Unknown" # Or a default movement (e.g., "Forward") if nothing is detected
